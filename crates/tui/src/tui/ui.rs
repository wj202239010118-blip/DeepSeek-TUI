...
async fn run_event_loop(
    ...
    loop {
        ...
        // #549 & P5: crossterm poll/read are synchronous blocking calls that
        // occupy a tokio worker thread. When the engine's event channel fills
        // (256 cap) and `tx_event.send().await` blocks the engine worker, we
        // hit a deadlock: UI blocked in console read, engine blocked in send.
        // spawn_blocking moves console I/O to a dedicated blocking thread so
        // both the engine and the UI loop stay schedulable.
        let maybe_evt = tokio::task::spawn_blocking(move || {
        ...
        if let Some(evt) = maybe_evt {
            ...
            if let Event::Resize(width, height) = evt {
                ...
                // P5: poll+read are blocking; OK here because resize drains are rare.
...
fn handle_composer_history_arrow(
    ...
    // When `composer_arrows_scroll` is on, the terminal may be converting
    // mouse-wheel events into arrow-key sequences.  In that mode Up/Down
    // always scroll the transcript — cursor-line movement is reserved for
    // terminals with real mouse capture where the user physically pressed
    // the arrow keys.

    match key.code {
        KeyCode::Up => {
            if app.composer_arrows_scroll {
...
fn mouse_over_composer(app: &App, mouse: MouseEvent) -> bool {
    mouse_hits_rect(mouse, app.viewport.last_composer_area)
}

fn mouse_over_transcript(app: &App, mouse: MouseEvent) -> bool {
    mouse_hits_rect(mouse, app.viewport.last_transcript_area)
}
...
fn handle_mouse_event(app: &mut App, mouse: MouseEvent) -> Vec<ViewEvent> {
    ...
    match mouse.kind {
        MouseEventKind::ScrollUp => {
            // Prioritise transcript: check it first so edge cases where
            // composer and transcript areas overlap resolve correctly.
            if mouse_over_transcript(app, mouse) {
            ...
            } else if mouse_over_composer(app, mouse) {
                // ScrollUp → show earlier content (decrease scroll offset)
                app.composer.composer_scroll_row =
                    app.composer.composer_scroll_row.saturating_sub(1);
                app.needs_redraw = true;
                }
        }
        MouseEventKind::ScrollDown => {
            if mouse_over_transcript(app, mouse) {
                ...
                // ScrollDown → show later content (increase scroll offset)
                app.composer.composer_scroll_row =
                    app.composer.composer_scroll_row.saturating_add(1);
                app.needs_redraw = true;
            }
        }
