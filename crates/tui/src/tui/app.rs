...
/// Pastes longer than this are shown as a pill `[Pasted text #N +X lines]`
/// in the Composer instead of inline text. The underlying `self.input` still
/// holds the `@.deepseek/pastes/paste-xxx.md` path so the submit pipeline
/// requires no changes.
const PASTE_PILL_THRESHOLD: usize = 200;

/// Metadata stored for a pasted-text pill so `ComposerWidget` can render the
/// pill without reading the file on every frame.
#[derive(Debug, Clone)]
...
pub struct ComposerState {
    ...
    /// Manual scroll row offset.  0 means auto-track the cursor;
    /// >0 means the user has scrolled to a specific wrapped row.
    pub composer_scroll_row: usize,
}
...
pub struct App {
    ...
    /// Counter incremented on every paste pill creation. Used for `#N` in the
    /// pill label so users can distinguish multiple pastes in one session.
    pub paste_counter: usize,
    /// Metadata keyed by the `@.deepseek/pastes/…` string inserted into
    /// `self.input`. The renderer looks up this map to display a pill instead
    /// of the raw path — no file I/O at render time.
    pub paste_pills: HashMap<String, PastePill>,
}
...
impl App {
    ...
    pub fn handle_resize(&mut self, _width: u16, _height: u16) {
        ...
        self.viewport.last_composer_area = None;
    ...
    pub fn insert_paste_text(&mut self, text: &str) {
        ...
        // Paste pill: for pastes longer than PASTE_PILL_THRESHOLD, save to a
        // file and display as `[Pasted text #N +X lines]` instead of inline
        // text. The underlying self.input stores the @mention path so the
        // submit pipeline requires no changes.
        if char_count(&normalized) > PASTE_PILL_THRESHOLD {
            let line_count = normalized.lines().count();
            let char_len = char_count(&normalized);
            ...
            if std::fs::create_dir_all(&pastes_dir).is_ok() {
                let file_path = self.workspace.join(&rel_path);
                if std::fs::write(&file_path, normalized.as_bytes()).is_ok() {
                    self.paste_counter += 1;
                    let mention = format!("@{rel_path}");
                    self.paste_pills.insert(
                        mention.clone(),
                        PastePill {
            ...
            // Fallback: if file write fails, insert inline (existing behaviour)
        }

        self.insert_str(&normalized);
        self.paste_burst.clear_after_explicit_paste();
        // Safety-net for the large-input cap (typed text, non-paste paths, etc.)
    ...
    pub fn move_cursor_left(&mut self) {
        ...
        self.composer.composer_scroll_row = 0;
        self.needs_redraw = true;
    }
    ...
    /// Move cursor up one logical line (across `\n` boundaries).
    /// Preserves the column offset from the start of the line,
    /// clamping to the length of the target line.
    pub fn move_cursor_up(&mut self) {
        ...
        let cursor_byte = byte_index_at_char(&self.input, cursor);

        // Find the byte position of the start of the current logical line.
        let line_start_byte = self.input[..cursor_byte]
            .rfind('\n')
            .map(|idx| idx + 1)
            .unwrap_or(0);

        if line_start_byte == 0 {
            // Already on the first line.
            return;
        }

        // Column offset (in chars) from the start of the current line.
        let col_chars = char_count(&self.input[line_start_byte..cursor_byte]);

        // Find the start of the previous line.
        let prev_line_end_byte = line_start_byte.saturating_sub(1); // the '\n'
        let prev_line_start_byte = self.input[..prev_line_end_byte]
            .rfind('\n')
            .map(|idx| idx + 1)
            .unwrap_or(0);

        let prev_line = &self.input[prev_line_start_byte..prev_line_end_byte];
        let prev_line_len = char_count(prev_line);
        let target_col = col_chars.min(prev_line_len);
        self.cursor_position = char_count(&self.input[..prev_line_start_byte]) + target_col;
        self.needs_redraw = true;
    }

    /// Move cursor down one logical line (across `\n` boundaries).
    /// Preserves the column offset from the start of the line,
    /// clamping to the length of the target line.
    pub fn move_cursor_down(&mut self) {
        ...
        let line_end_byte = self.input[cursor_byte..]
            .find('\n')
            .map(|rel| cursor_byte + rel)
        ...
        // Start of the next line (after the '\n').
        let next_line_start_byte = line_end_byte + 1;
        if next_line_start_byte >= self.input.len() {
            self.cursor_position = total_chars;
            self.needs_redraw = true;
            return;
        }

        let next_line_end_byte = self.input[next_line_start_byte..]
            .find('\n')
            .map(|rel| next_line_start_byte + rel)
            .unwrap_or(self.input.len());

        let next_line = &self.input[next_line_start_byte..next_line_end_byte];
        let next_line_len = char_count(next_line);
        let target_col = col_chars.min(next_line_len);
        self.cursor_position =
            char_count(&self.input[..next_line_start_byte]) + target_col;
        self.needs_redraw = true;
    }
    ...
    /// Returns the pill label for display if the current input is a paste pill,
    /// e.g. `"[Pasted text #1 +42 lines]"`. Returns `None` when the input is
    /// normal text so callers can fall back to `composer_display_input`.
    pub fn composer_paste_pill_label(&self) -> Option<String> {
        let input = self.history_search_query().unwrap_or(&self.input);
        self.paste_pills.get(input).map(|pill| {
            format!("[Pasted text #{} +{} lines]", pill.id, pill.line_count)
        })
    }
