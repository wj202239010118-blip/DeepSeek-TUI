...
mcpServer.registerTool(
	'browser_inspect_element',
	{
		...
		inputSchema: {
			index: z.number().int().min(0).describe('Element index as shown in browser_get_map'),
		},
	},
	async ({ index }) => {
		try {
...
mcpServer.registerTool(
	...
	{
		description:
			'Open a URL in a browser tab. By default, reuses an existing tab with the same origin+pathname instead of creating a new one. Set reuse=false to force a new tab.',
		inputSchema: {
			url: z.string().describe('Full URL to open'),
			timeout: z.number().optional().describe('Load timeout ms (default 15000)'),
			reuse: z.boolean().optional().describe('Reuse existing tab (default true)'),
		},
	},
	async ({ url, timeout = 15000, reuse = true }) => {
		try {
			const data = await hub.executePrimitiveOp('open_tab', { url, timeout, reuse });
			return { content: [{ type: 'text', text: data }] };
		} catch (err) {
			return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
		}
...
mcpServer.registerTool(
	...
	{
		description:
			'List all open browser tabs with id, url, title, active/pinned status. Use before opening a new tab to avoid duplicates.',
		inputSchema: {},
	},
	async () => {
		try {
			const data = await hub.executePrimitiveOp('list_tabs', {});
			return { content: [{ type: 'text', text: data }] };
		} catch (err) {
			return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
		}
...
mcpServer.registerTool(
	...
	{
		description:
			'Switch to a tab by tabId or URL substring match.',
		inputSchema: {
			tabId: z.number().int().optional().describe('Tab ID to switch to'),
			url: z.string().optional().describe('URL substring to match'),
		},
	},
	async ({ tabId, url }) => {
		try {
			const data = await hub.executePrimitiveOp('switch_tab', { tabId, url });
			return { content: [{ type: 'text', text: data }] };
		} catch (err) {
			return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
		}
...
mcpServer.registerTool(
	...
	{
		description:
			'Close a tab by tabId. If omitted, closes the active tab.',
		inputSchema: {
			tabId: z.number().int().optional().describe('Tab ID to close. If omitted, closes active tab.'),
		},
	},
	async ({ tabId }) => {
		try {
			const data = await hub.executePrimitiveOp('close_tab', { tabId });
			return { content: [{ type: 'text', text: data }] };
		} catch (err) {
			return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
		}
...
mcpServer.registerTool(
	'browser_click',
	{
		...
		inputSchema: { index: z.number().int().min(0).describe('Element index to click') },
	},
	async ({ index }) => {
		try {
...
mcpServer.registerTool(
	'browser_type',
	{
		...
		inputSchema: {
			index: z.number().int().min(0).describe('Element index to type into'),
